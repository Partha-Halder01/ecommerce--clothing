...
const DEFAULT_SIZES = ["50ml", "100ml", "150ml", "200ml"]
const DEFAULT_COLORS = [
  { name: "Beige", value: "#d4c4b0" },
  { name: "Navy", value: "#1C1615" },
  { name: "Sapphire", value: "#6B635E" },
...
interface FormData {
  name: string; slug: string; description: string; category: string; price: string; original_price: string; stock: string;
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    if (formData.sizes.length === 0) {
      toast({
        ...
        description: "Select at least one volume before saving this product.",
        variant: "destructive",
      })
      return
    }
    ...
    const payload = {
      ...
      gallery_images: formData.gallery_images.length > 0 ? formData.gallery_images.map(img => img.url) : undefined,
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-light text-[#1C1615] uppercase tracking-widest" style={{ fontFamily: "var(--font-playfair)" }}>Products</h1>
        ...
        <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <form onSubmit={(e) => handleSubmit(e, isEditOpen)} className="flex-1 overflow-y-auto space-y-4 pr-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isEditOpen) {
                      const slug = e.target.value.toLowerCase().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-').replace(/^-+|-+$/g, '');
              ...
              <div>
                <label className="text-sm font-medium mb-1 block">Product Slug (SEO Friendly)</label>
                <Input
                  value={formData.slug}
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  required
                />
                <p className="text-xs text-[#6B635E] mt-1">Actual selling price</p>
              </div>
              <div>
                ...
                <Input
                  ...
                  placeholder="0"
                />
                <p className="text-xs text-[#6B635E] mt-1">MRP / Strikethrough price</p>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="flex gap-2 items-center flex-wrap">
                ...
                <span className="text-[#6B635E]/70 text-sm">or</span>
                <Input
              ...
              {formData.image_url && (
                <div className="relative w-16 h-16 rounded overflow-hidden bg-[#F4F1EB] mt-2 inline-block">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {formData.gallery_images.map((img, i) => (
                    <div key={i} className="relative group">
                      <div className="w-16 h-16 rounded overflow-hidden bg-[#F4F1EB]">
              ...
              {formData.gallery_images.length > 0 && <p className="text-xs text-[#6B635E] mt-8">Select a color below each image to show it only for that color, or leave as "All Colors"</p>}
            </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                {availableSizes.map((size) => (
                  <button key={size} type="button" onClick={() => toggleSize(size)}
                    className={`px-2.5 py-1 rounded border text-xs font-medium transition-colors ${formData.sizes.includes(size) ? "bg-[#1C1615] text-[#FCFBF8] border-[#1C1615]" : "border-gray-300 hover:border-[#1C1615]"
                      }`}>{size}</button>
                ))}
              </div>
              <div className="flex gap-2 items-center">
                <Input
                  placeholder="Custom volume (e.g., 75ml)"
                  value={newSize}
                  onChange={(e) => setNewSize(e.target.value)}
                  className="w-28"
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addCustomSize(); } }}
                />
                <Button type="button" variant="outline" size="sm" onClick={addCustomSize}>Add Volume</Button>
              </div>
              {formData.sizes.length === 0 && (
                <p className="text-xs text-red-600 mt-2">Please select at least one volume to publish this product.</p>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-[#6B635E] mb-3">Select products to display in "You May Also Like" section</p>
              <div className="max-h-48 overflow-y-auto border rounded-lg p-3 space-y-2 bg-[#F4F1EB]">
                {products.filter(p => p.id !== editingProduct?.id).length === 0 ? (
                  <p className="text-sm text-[#6B635E]/70 text-center py-4">No other products available</p>
                ) : (
                  products.filter(p => p.id !== editingProduct?.id).map((p) => (
                    <label key={p.id} className="flex items-center gap-3 p-2 hover:bg-white rounded cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 text-[#6B635E] border-gray-300 rounded focus:ring-[#D8B4A0]"
                      />
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-[#6B635E]/70 ml-2">₹{p.price}</span>
                        </div>
                      </div>
                    </label>
                  ))
              ...
              {formData.related_products.length > 0 && (
                <p className="text-xs text-[#6B635E] mt-2">{formData.related_products.length} product(s) selected</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {formData.faqs.map((faq, idx) => (
                  <div key={idx} className="bg-[#F4F1EB] rounded-lg p-3 border">
