...
export default function AdminOrders() {
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      'Shipped': 'bg-[#D8B4A0]/20 text-[#1C1615]',
    ...
    return colors[status] || 'bg-[#F4F1EB] text-gray-800'
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-6 md:mb-8">
        <h1 className="text-2xl md:text-3xl font-light text-[#1C1615] uppercase tracking-widest" style={{ fontFamily: "var(--font-playfair)" }}>Orders</h1>
                    ...
                    <div className="space-y-2">
                      {Array.isArray(order.items) && order.items.length > 0 ? order.items.map((item: any, idx: number) => (
                        <div key={idx} className="flex gap-2 pb-2 border-b last:border-0 last:pb-0">
                          {item.image && (
                            <div className="relative w-10 h-10 rounded overflow-hidden bg-[#F4F1EB] flex-shrink-0">
