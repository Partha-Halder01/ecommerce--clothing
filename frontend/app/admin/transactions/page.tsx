    ...
    <div className="p-4 sm:p-6 lg:p-8 bg-[#f8f9fc] min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[#1C1615]">Transaction History</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-none p-4 border shadow-sm">
          ...
          <div className="bg-white rounded-none p-4 border shadow-sm">
            <div className="flex items-center gap-2">
              <CreditCard className="w-4 h-4 text-[#D8B4A0]" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#F4F1EB] border-b">
                  <tr>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Order ID</th>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Customer</th>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Amount</th>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Payment Method</th>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Payment ID</th>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Status</th>
                    <th className="text-left p-4 text-sm font-medium text-[#6B635E]">Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {filteredTransactions.map((t) => (
                    <tr key={t.order_id} className="hover:bg-[#F4F1EB]">
                      ...
                      <td className="p-4">
                        {t.payment_method === "cod" ? (
                          ...
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-[#D8B4A0]/20 text-[#1C1615]">
                      ...
                      <td className="p-4 text-sm">
                        {t.payment_id ? (
                          <code className="bg-[#F4F1EB] px-2 py-1 rounded text-xs font-mono">{t.payment_id}</code>
                        ) : (
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${t.status === "Delivered" ? "bg-green-100 text-green-700" :
                          t.status === "Processing" ? "bg-[#D8B4A0]/20 text-[#1C1615]" :
