...
export default function CheckoutPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState("")  // Will be set based on enabled methods
  ...
  const [paymentSettings, setPaymentSettings] = useState({ cod_enabled: true, online_enabled: true })
    ...
    const fetchPaymentSettings = async () => {
      try {
        ...
        if (res.ok) {
          ...
          } else if (data.online_enabled) {
            setPaymentMethod("online")
          }
        }
      } catch (e) {
        // Default to COD if can't fetch
        setPaymentMethod("cod")
      }
    }
              ...
              <section className="bg-card rounded-xl p-5 sm:p-6 border border-border">
                ...
                <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-3">
                  ...
                  {!paymentSettings.cod_enabled && !paymentSettings.online_enabled && (
                    <p className="text-muted-foreground text-center py-4">No payment methods available</p>
                  )}
                </RadioGroup>
              </section>
