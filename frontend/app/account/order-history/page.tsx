...
export default function OrderHistoryPage() {
    ...
    return (
        <>
            <Header />
            <main className="min-h-screen bg-[#FCFBF8] py-8 sm:py-12">
                <div className="container mx-auto px-4 max-w-4xl">
                    {/* Header */}
                    <div className="flex items-center gap-4 mb-6">
                        <Link href="/account" className="text-[#6B635E] hover:text-[#1C1615]">
                            <ArrowLeft className="w-5 h-5" />
                        </Link>
                        <div>
                            <h1 className="text-2xl font-semibold text-[#1C1615]">Order History</h1>
                            <p className="text-[#6B635E] text-sm">Your completed orders</p>
                        </div>
                    </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="bg-white rounded-sm p-12 text-center">
                            ...
                            <Link href="/shop" className="text-[#6B635E] hover:underline">Continue Shopping</Link>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {orders.map((order) => (
                                <div key={order.id} className="bg-white rounded-sm border shadow-sm overflow-hidden">
                                    {/* Order Header */}
                                    <div className="p-4 sm:p-5 border-b bg-gray-50">
                                        <div className="flex flex-wrap items-center justify-between gap-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-[#1C1615]">Order #{order.id}</p>
                                                    <p className="text-sm text-green-600">Delivered</p>
                                                </div>
                                            </div>
                                            <div className="text-lg font-semibold text-green-600">₹{order.total.toFixed(2)}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 sm:p-5">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                            <div className="flex items-center gap-2 text-sm">
                                                ...
                                                <span className="text-[#1C1615]">{formatDate(order.created_at)}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-sm">
                                        ...
                                        <div className="border-t pt-4">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {order.items.slice(0, 4).map((item: any, idx: number) => (
                                                    <div key={idx} className="flex items-center gap-2 bg-gray-50 rounded-sm p-2">
                                                        {item.image && (
                                                            <img src={item.image} alt={item.name} className="w-10 h-10 object-cover rounded" />
                                                        )}
                                                        <div className="text-xs">
                                                            <p className="font-medium text-[#1C1615] line-clamp-1">{item.name}</p>
                                                            <p className="text-gray-500">x{item.quantity}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                                {order.items.length > 4 && (
                                                    <div className="flex items-center justify-center bg-gray-100 rounded-sm px-3">
