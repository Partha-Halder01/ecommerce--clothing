...
# Load environment variables from explicit path (for gunicorn daemon)
...
SHIPROCKET_PASSWORD = os.getenv('SHIPROCKET_PASSWORD', os.getenv('Shiprocket', ''))
from shiprocket_client import client as shiprocket_client
...
from models import (
    ...
    # Product operations
    create_product, get_all_products, get_product_by_id, get_product_by_slug, update_product, delete_product,
    # Order operations
    create_order, get_all_orders, get_user_orders, get_order_by_id, update_order_status,
    # Dashboard
    get_dashboard_stats, get_weekly_stats,
    # Categories
...
def register_with_otp():
    ...
    try:
        ...
        send_email_otp(data['email'], otp_code, 'register')
...
def send_email_otp(to_email, otp_code, purpose='login'):
    ...
    from_name = os.getenv('SMTP_FROM_NAME', 'Inyou Store')
    ...
    subject_map = {
        'login': 'Your Inyou Login OTP',
        'register': 'Verify Your Inyou Account',
        'password_reset': 'Reset Your Inyou Password'
    }
    
    subject = subject_map.get(purpose, 'Your Inyou OTP')
    
    html_body = f"""
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #0D2440; text-align: center;">Inyou</h2>
...
@app.route('/api/products/slug/<string:slug>', methods=['GET'])
def get_product_slug(slug):
    ...
    try:
        ...
        print(f"Get product slug error: {e}")
        return jsonify({'detail': str(e)}), 500

# ==================== SHIPROCKET ADMIN ROUTES ====================

@app.route('/api/admin/shiprocket/serviceability', methods=['GET'])
@token_required
@admin_required
def shiprocket_serviceability(current_user):
    try:
        pickup_postcode = request.args.get('pickup_postcode')
        delivery_postcode = request.args.get('delivery_postcode')
        weight = float(request.args.get('weight', '0.5'))
        cod = int(request.args.get('cod', '0'))
        if not pickup_postcode or not delivery_postcode:
            return jsonify({'detail': 'pickup_postcode and delivery_postcode are required'}), 400
        data = shiprocket_client.serviceability(pickup_postcode, delivery_postcode, weight, cod)
        return jsonify(data)
    except requests.HTTPError as e:
        return jsonify({'detail': f'Shiprocket error: {e.response.text}'}), e.response.status_code
    except Exception as e:
        print(f"Shiprocket serviceability error: {e}")
        return jsonify({'detail': str(e)}), 500

@app.route('/api/admin/shiprocket/track', methods=['GET'])
@token_required
@admin_required
def shiprocket_track(current_user):
    try:
        awb = request.args.get('awb')
        if not awb:
            return jsonify({'detail': 'awb is required'}), 400
        data = shiprocket_client.track(awb)
        return jsonify(data)
    except requests.HTTPError as e:
        return jsonify({'detail': f'Shiprocket error: {e.response.text}'}), e.response.status_code
    except Exception as e:
...
@app.route('/api/admin/shiprocket/estimate', methods=['GET'])
@token_required
@admin_required
def shiprocket_estimate(current_user):
    try:
        pickup_postcode = request.args.get('pickup_postcode')
        delivery_postcode = request.args.get('delivery_postcode')
        weight = float(request.args.get('weight', '0.5'))
        if not pickup_postcode or not delivery_postcode:
            return jsonify({'detail': 'pickup_postcode and delivery_postcode are required'}), 400
        data = shiprocket_client.estimate(pickup_postcode, delivery_postcode, weight)
        return jsonify(data)
    except requests.HTTPError as e:
        return jsonify({'detail': f'Shiprocket error: {e.response.text}'}), e.response.status_code
    except Exception as e:
        print(f"Shiprocket estimate error: {e}")
        return jsonify({'detail': str(e)}), 500

@app.route('/api/admin/shiprocket/orders/create', methods=['POST'])
@token_required
@admin_required
def shiprocket_orders_create(current_user):
    try:
        data = request.get_json()
        result = shiprocket_client.create_order(data)
        return jsonify(result)
    except requests.HTTPError as e:
        return jsonify({'detail': e.response.text}), e.response.status_code
    except Exception as e:
        return jsonify({'detail': str(e)}), 500

@app.route('/api/admin/shiprocket/orders/cancel', methods=['POST'])
@token_required
@admin_required
def shiprocket_orders_cancel(current_user):
    try:
        data = request.get_json()
        sr_order_id = int(data.get('sr_order_id'))
        result = shiprocket_client.cancel_order(sr_order_id)
        return jsonify(result)
    except requests.HTTPError as e:
        return jsonify({'detail': e.response.text}), e.response.status_code
    except Exception as e:
...
@app.route('/api/admin/dashboard/weekly', methods=['GET'])
@token_required
@admin_required
def admin_weekly_stats(current_user):
...
def admin_create_product(current_user):
    ...
    try:
        ...
        if not isinstance(raw_sizes, list):
            return jsonify({'detail': 'At least one volume is required'}), 400
        cleaned_sizes = [str(size).strip() for size in raw_sizes if str(size).strip()]
...
def admin_update_order(current_user, order_id):
    ...
    try:
        ...
        # Determine if we should trigger Shiprocket order creation
        status = data['status']
        if status == 'Processing' and existing.get('status') != 'Processing' and not existing.get('shiprocket_order_id'):
            # Trigger Shiprocket Order Creation
            try:
                # Prepare payload
                # Use default dimensions (Shiprocket API needs dimensions in cm and weight in kg)
                items = existing.get('items', [])
                order_items = []
                for item in items:
                    order_items.append({
                        "name": item.get('name', 'Product'),
                        "sku": str(item.get('id', 'N/A')),
                        "units": item.get('quantity', 1),
                        "selling_price": item.get('price', 0),
                        "discount": 0,
                    })

                # Try to parse shipping address if it's JSON, else extract basic parts (Assuming simple text address for now)
                # Shiprocket needs structured address. We try best effort splitting.
                address_parts = existing.get('shipping_address', 'N/A').split(',')
                ...
                pincode = "110001" # Defaulting if we can't extract (needs better structure in UI)
                import re
                pin_match = re.search(r'\b\d{6}\b', existing.get('shipping_address', ''))
                if pin_match:
                    pincode = pin_match.group(0)

                order_date = existing.get('created_at', datetime.now())
                ...
                payload = {
                    "order_id": f"INYOU-{existing['id']}",
                    "order_date": order_date_str,
                    "pickup_location": "Primary", # Default location in Shiprocket
                    "billing_customer_name": existing.get('customer_name', 'Guest'),
                    "billing_last_name": "",
                    "billing_address": existing.get('shipping_address', 'N/A')[:255],
                    "billing_city": city,
                    "billing_pincode": pincode,
                    "billing_state": state,
                    "billing_country": "India",
                    "billing_email": existing.get('customer_email', 'no-email@inyou.luxury'),
                    "billing_phone": existing.get('customer_phone', '9999999999'),
                    "shipping_is_billing": True,
                    "order_items": order_items,
                    "payment_method": "Prepaid" if existing.get('payment_method') != 'cod' else "COD",
                    "sub_total": existing.get('total', 0),
                    "length": 10,
                    "breadth": 10,
                    "height": 10,
                ...
                from shiprocket_client import client as shiprocket_client
                sr_response = shiprocket_client.create_order(payload)
                
                # Extract IDs
                sr_order_id = sr_response.get('order_id')
                sr_shipment_id = sr_response.get('shipment_id')
                sr_awb = sr_response.get('awb_code')
                
                # Save to database
                from models import update_order_shiprocket_details
                if sr_order_id:
                    update_order_shiprocket_details(order_id, sr_order_id, sr_shipment_id, sr_awb)

            except Exception as sr_error:
                print(f"Failed to create Shiprocket Order: {sr_error}")
                # We update the status anyway, but logging the failure
                # Optionally, return a partial error or just proceed
...
def admin_create_category(current_user):
    ...
    try:
        ...
        image_url = data.get('image_url')
        category_id = create_category(data['name'], data.get('description'), parent_id if parent_id else None, image_url)
...
def admin_update_category(current_user, category_id):
    ...
    try:
        data = request.get_json()
        update_category(category_id, data.get('name'), data.get('description'), data.get('is_active'), data.get('parent_id'), data.get('image_url'))
...
def submit_contact():
    ...
    try:
        ...
        # Fallback subject for older clients
        subject = data.get('subject') or 'Contact Inquiry'
